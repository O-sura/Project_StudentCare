...
function dropdownFilter(option) {
  ...
  xhr.open(
    "GET",
    "http://localhost/StudentCare/CounselorAnnouncement/dropdown_handler/?filter=" +
      option +
      "&userID=" +
      loggedInUser,
    true
  ...
  xhr.onload = () => {
    if (xhr.status === 200) {
      ...
      for (var i = 0; i < searchRes.length; i++) {
        ...
        let post = new CounselorAnnouncementPost(
          result.post_id,
          result.post_desc,
          result.posted_date,
          result.fullname,
...
searchBar.addEventListener("input", () => {
  ...
  xhr.open(
    "GET",
    "http://localhost/StudentCare/CounselorAnnouncement/search_posts/?query=" +
      searchBar.value,
    true
  );
