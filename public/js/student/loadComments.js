...
function loadComments(listing_id) {
  ...
  xhr.open(
    "GET",
    "http://localhost/StudentCare/Student_facility/comment_loader/?id=" +
      listing_id +
      "&rating=" +
      listing_rating +
      "&feedback=" +
...
function helpful(review_id, value) {
  ...
  xhr.open(
    "GET",
    "http://localhost/StudentCare/Student_facility/comment_helpful_handler/?id=" +
      review_id +
      "&value=" +
      value +
      "&listing=" +
  ...
  xhr.onload = () => {
    if (xhr.status === 200) {
      ...
      let radios = document.getElementsByName("helpful");
